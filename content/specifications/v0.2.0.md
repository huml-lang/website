...
* Blank lines are ignored (except in multi-line strings).
...
|             | **Multi-line string**: Wrapped in `"""` (three double quotes). Newlines at the beginning and end will be stripped.                  |                                                           |
|             | Characters inside a multi-line string cannot be escaped. Escape sequences with backslashes like `\n` are treated literally. |                                                           |
