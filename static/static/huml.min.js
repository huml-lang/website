...
`,t:"\t",r:"\r",f:"\f",v:"\v"},F=[["0x",16],["0o",8],["0b",2]];class K extends Error{constructor(z,H){super(`line ${H}: ${z}`);this.name="HUMLError",this.line=H}}class U{constructor(z){this.data=z,this.pos=0,this.line=1}parse(){if(this.data.length===0)throw Error("empty document is undefined");if(this.peekString("%HUML")){if(this.advance(5),!this.done()&&this.data[this.pos]===" "){this.advance(1);let J=this.pos;while(!this.done()&&![" ",`
`,"#"].includes(this.data[this.pos]))this.pos++;if(this.pos>J){let Q=this.data.substring(J,this.pos);if(Q!=="v0.2.0")throw this.error(`unsupported version '${Q}'. expected 'v0.2.0'`)}}this.consumeLine()}if(this.skipBlankLines(),this.done())throw this.error("empty doc is undefined");if(this.getCurIndent()!==0)throw this.error("root element must not be indented");if(this.peekString("::"))throw this.error("'::' indicator not allowed at document root");if(this.peekString(":")&&!this.hasKeyValuePair())throw this.error("':' indicator not allowed at document root");let z=this.getRootType(),D={[Z.INLINE_DICT]:()=>this.assertRootEnd(this.parseInlineVectorContents(Z.INLINE_DICT),"root inline dict"),[Z.MULTILINE_DICT]:()=>this.parseMultilineDict(0),[Z.EMPTY_LIST]:()=>{return this.advance(2),this.consumeLine(),this.assertRootEnd([],"root list")},[Z.EMPTY_DICT]:()=>{return this.advance(2),this.consumeLine(),this.assertRootEnd({},"root dict")},[Z.MULTILINE_LIST]:()=>this.parseMultilineList(0),[Z.INLINE_LIST]:()=>this.assertRootEnd(this.parseInlineVectorContents(Z.INLINE_LIST),"root inline list"),[Z.SCALAR]:()=>{let J=this.parseValue(0);return this.consumeLine(),this.assertRootEnd(J,"root scalar value")}}[z];if(!D)throw this.error("internal error: unknown document type");return D()}getRootType(){if(this.hasKeyValuePair())return this.hasInlineDictAtRoot()?Z.INLINE_DICT:Z.MULTILINE_DICT;let z=[[()=>this.peekString("[]"),Z.EMPTY_LIST],[()=>this.peekString("{}"),Z.EMPTY_DICT],[()=>this.peekChar(this.pos)==="-",Z.MULTILINE_LIST],[()=>this.hasInlineListAtRoot(),Z.INLINE_LIST]];for(let[H,D]of z)if(H())return D;return Z.SCALAR}assertRootEnd(z,H){if(this.skipBlankLines(),!this.done())throw this.error(`unexpected content after ${H}`);return z}parseMultilineDict(z){let H={};while(!0){if(this.skipBlankLines(),this.done())break;let D=this.getCurIndent();if(D<z)break;if(D!==z)throw this.error(`bad indent ${D}, expected ${z}`);if(!this.isKeyStart())throw this.error(`invalid character '${this.data[this.pos]}', expected key`);let J=this.parseKey();if(J in H)throw this.error(`duplicate key '${J}' in dict`);let Q=this.parseIndicator(),W;if(Q===":"){this.assertSpace("after ':'");let $=this.peekString('"""');if(W=this.parseValue(D),!$)this.consumeLine()}else W=this.parseVector(D+2);H[J]=W}return H}parseMultilineList(z){let H=[];while(!0){if(this.skipBlankLines(),this.done())break;let D=this.getCurIndent();if(D<z)break;if(D!==z)throw this.error(`bad indent ${D}, expected ${z}`);if(this.data[this.pos]!=="-")break;this.advance(1),this.assertSpace("after '-'");let J;if(this.peekString("::"))this.advance(2),J=this.parseVector(D+2);else J=this.parseValue(D),this.consumeLine();H.push(J)}return H}getMultilineVectorType(z){if(this.skipBlankLines(),this.done())throw this.error("ambiguous empty vector after '::'. Use [] or {}.");if(this.getCurIndent()<z)throw this.error("ambiguous empty vector after '::'. Use [] or {}.");return this.data[this.pos]==="-"?"list":"dict"}parseVector(z){let H=this.pos;if(this.skipSpaces(),this.done()||this.data[this.pos]===`
`||this.data[this.pos]==="#")return this.pos=H,this.consumeLine(),this.getMultilineVectorType(z)==="list"?this.parseMultilineList(z):this.parseMultilineDict(z);return this.pos=H,this.assertSpace("after '::'"),this.parseInlineVector()}parseInlineVector(){if(this.peekString("[]"))return this.advance(2),this.consumeLine(),[];if(this.peekString("{}"))return this.advance(2),this.consumeLine(),{};return this.hasInlineDict()?this.parseInlineVectorContents(Z.INLINE_DICT):this.parseInlineVectorContents(Z.INLINE_LIST)}parseInlineVectorContents(z){let H=z===Z.INLINE_DICT?{}:[],D=!0;while(!this.done()&&this.data[this.pos]!==`
`&&this.data[this.pos]!=="#"){if(!D)this.expectComma();if(D=!1,z===Z.INLINE_DICT){let J=this.parseKey();if(this.done()||this.data[this.pos]!==":")throw this.error("expected ':' in inline dict");if(this.advance(1),this.assertSpace("in inline dict"),J in H)throw this.error(`duplicate key '${J}' in dict`);H[J]=this.parseValue(0)}else H.push(this.parseValue(0));if(!this.done()&&this.data[this.pos]===" "){let J=this.pos+1;while(J<this.data.length&&this.data[J]===" ")J++;if(J<this.data.length&&this.data[J]===",")this.skipSpaces();else break}}return this.consumeLine(),H}parseKey(){if(this.skipSpaces(),this.peekChar(this.pos)==='"')return this.parseString();else if(!this.isKeyStart())throw this.error(`invalid character '${this.data[this.pos]}', expected key`);let z=this.pos;while(!this.done()&&(this.isAlphaNum(this.data[this.pos])||this.data[this.pos]==="-"||this.data[this.pos]==="_"))this.pos++;if(this.pos===z)throw this.error("expected a key");return this.data.substring(z,this.pos)}parseIndicator(){if(this.done()||this.data[this.pos]!==":")throw this.error("expected ':' or '::' after key");if(this.advance(1),!this.done()&&this.data[this.pos]===":")return this.advance(1),"::";return":"}parseValue(z){if(this.done())throw this.error("unexpected end of input, expected a value");let H=this.data[this.pos];if(H==='"')return this.peekString('"""')?this.parseMultilineString(z):this.parseString();for(let[D,J]of j)if(this.peekString(D))return this.advance(D.length),J;if(H==="+"){if(this.advance(1),this.peekString("inf"))return this.advance(3),1/0;if(this.isDigit(this.peekChar(this.pos)))return this.pos--,this.parseNumber();throw this.error("invalid character after '+'")}if(H==="-"){if(this.advance(1),this.peekString("inf"))return this.advance(3),-1/0;if(this.isDigit(this.peekChar(this.pos)))return this.pos--,this.parseNumber();throw this.error("invalid character after '-'")}if(this.isDigit(H))return this.parseNumber();throw this.error(`unexpected character '${H}' when parsing value`)}parseString(){this.advance(1);let z="";while(!this.done()){let H=this.data[this.pos];if(H==='"')return this.advance(1),z;if(H===`
`)throw this.error("newlines not allowed in single-line strings");if(H==="\\"){if(this.advance(1),this.done())throw this.error("incomplete escape sequence");let D=this.data[this.pos];if(B.hasOwnProperty(D))z+=B[D];else throw this.error(`invalid escape character '\\${D}'`)}else z+=H;this.advance(1)}throw this.error("unclosed string")}parseMultilineString(z){let H=this.data.substring(this.pos,this.pos+3);this.advance(3),this.consumeLine();let D=(Q,W)=>{let $=z+2;if(Q.length>=$&&this.isSpaceString(Q.substring(0,$)))return Q.substring($);return Q},J=[];while(!this.done()){let Q=this.pos,W=0;while(!this.done()&&this.data[this.pos]===" ")W++,this.pos++;if(this.peekString(H)){if(W!==z)throw this.error(`multiline closing delimiter must be at same indentation as the key (${z} spaces)`);return this.advance(3),this.consumeLine(),J.join(`
`)}this.pos=Q;let $=this.consumeLineContent();J.push(D($,W))}throw this.error("unclosed multiline string")}parseNumber(){let z=this.pos,H=this.peekChar(this.pos);if(H==="+"||H==="-")this.advance(1);for(let[Q,W]of F)if(this.peekString(Q))return this.parseBase(z,W,Q);let D=!1;while(!this.done()){let Q=this.data[this.pos];if(this.isDigit(Q)||Q==="_")this.advance(1);else if(Q===".")D=!0,this.advance(1);else if(["e","E"].includes(Q)){if(D=!0,this.advance(1),["+","-"].includes(this.peekChar(this.pos)))this.advance(1)}else break}let J=this.data.substring(z,this.pos).replace(/_/g,"");return D?parseFloat(J):parseInt(J,10)}parseBase(z,H,D){this.advance(D.length);let J=this.pos,W={16:(G)=>this.isHex(G),8:(G)=>G>="0"&&G<="7",2:(G)=>["0","1"].includes(G)}[H];while(!this.done()&&W(this.data[this.pos]))this.advance(1);if(this.pos===J)throw this.error("invalid number literal, requires digits after prefix");let $=this.data[z]==="-"?-1:1,O=this.data.substring(J,this.pos).replace(/_/g,"");return $*parseInt(O,H)}skipBlankLines(){while(!this.done()){let z=this.pos;if(this.skipSpaces(),this.done()){if(this.pos>z)throw this.error("trailing spaces are not allowed");return}if(![`
...
`).slice(1).some((O)=>{let G=O.trim();return G&&!G.startsWith("#")})}isKeyStart(){return!this.done()&&(this.data[this.pos]==='"'||this.isAlpha(this.data[this.pos]))}done(){return this.pos>=this.data.length}advance(z){this.pos+=z}skipSpaces(){while(!this.done()&&this.data[this.pos]===" ")this.advance(1)}peekString(z){return this.pos+z.length<=this.data.length&&this.data.startsWith(z,this.pos)}peekChar(z){return z>=0&&z<this.data.length?this.data[z]:"\x00"}isDigit(z){return z>="0"&&z<="9"}isAlpha(z){return z>="a"&&z<="z"||z>="A"&&z<="Z"}isAlphaNum(z){return this.isAlpha(z)||this.isDigit(z)}isHex(z){return this.isDigit(z)||z>="a"&&z<="f"||z>="A"&&z<="F"}isSpaceString(z){return z.trim()===""}error(z){return new K(z,this.line)}}function M(z){if(typeof z!=="string")throw TypeError("HUML input must be of type string");return new U(z).parse()}var C=/^[a-zA-Z][a-zA-Z0-9_-]*$/;function X(z,H,D,J=!1){if(z===null||z===void 0){D[D.length-1]+="null";return}let Q=typeof z;if(Q==="boolean")D[D.length-1]+=String(z);else if(Q==="number")D[D.length-1]+=N(z);else if(Q==="string")_(z,H,D);else if(Array.isArray(z))w(z,H,D,J);else if(Q==="object")f(z,H,D,J);else throw Error(`Unsupported type: ${Q}`)}function N(z){if(Number.isNaN(z))return"nan";if(z===1/0)return"inf";if(z===-1/0)return"-inf";return String(z)}function _(z,H,D){if(z.includes(`
`)){let J=H-2,Q=H;D[D.length-1]+='"""';let W=z.split(`
`);if(W[W.length-1]==="")W.pop();W.forEach(($)=>{D.push(" ".repeat(Q)+$)}),D.push(" ".repeat(J)+'"""')}else D[D.length-1]+=JSON.stringify(z)}function w(z,H,D,J=!1){if(z.length===0){D[D.length-1]+="[]";return}let Q=J?0:H;z.forEach((W,$)=>{if(D.push(" ".repeat(Q)+"- "),q(W))D[D.length-1]+="::",X(W,Q+2,D);else X(W,Q,D)})}function f(z,H,D,J=!1){let Q=Object.entries(z);if(Q.length===0){D[D.length-1]+="{}";return}Q.sort(([$],[O])=>$.localeCompare(O));let W=J?0:H;Q.forEach(([$,O],G)=>{A($,O,W,D)})}function A(z,H,D,J){J.push(" ".repeat(D)+g(z));let Q=q(H),W=h(H);if(Q)J[J.length-1]+=W?":: ":"::";else J[J.length-1]+=": ";X(H,D+2,J)}function q(z){return z!==null&&(Array.isArray(z)||typeof z==="object"&&z.constructor===Object)}function h(z){if(Array.isArray(z))return z.length===0;if(typeof z==="object"&&z!==null)return Object.keys(z).length===0;return!1}function g(z){return C.test(z)?z:JSON.stringify(z)}function V(z,H){let D=[];if(H&&H.includeVersion)D.push("%HUML v0.2.0"),D.push("");return X(z,0,D,!0),D.push(""),D.join(`
`)}export{V as stringify,M as parse};
